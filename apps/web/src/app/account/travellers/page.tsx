...
import { addTraveller, deleteTraveller, listTravellers, type Traveller } from "@/lib/account-api";
...
export default function TravellersPage() {
  ...
  async function remove(id: string) {
    ...
    try {
      setList(await deleteTraveller(id));
    } catch {
      setList(prev);
    }
  }
  ...
  return (
    <div>
      ...
      {loading ? (
        <div className="space-y-3">{[0, 1].map((i) => <div key={i} className="h-20 animate-pulse rounded-2xl bg-slate-100" />)}</div>
      ) : list.length === 0 ? (
        <div className="rounded-2xl border border-[var(--hairline)] bg-surface p-12 text-center text-muted">
          No saved travellers yet. Add one for faster checkout.
        </div>
      ) : (
            ...
            <div key={t.id} className="flex items-center justify-between gap-4 rounded-2xl border border-[var(--hairline)] bg-surface p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-sm text-muted">CNIC {t.cnic} · {t.gender}{t.dob ? ` · ${t.dob}` : ""}</div>
                </div>
              </div>
              <button
                onClick={() => remove(t.id)}
...
function AddTraveller({ onClose, onSaved }: { onClose: () => void; onSaved: (l: Traveller[]) => void }) {
  const [f, setF] = useState({ name: "", relation: "Family", cnic: "", dob: "", gender: "Male" as Traveller["gender"] });
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const valid = f.name.trim().length > 2;
  const set = (k: keyof typeof f) => (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) =>
    setF({ ...f, [k]: e.target.value as never });

  async function add() {
    setBusy(true);
    setError(null);
    try {
      onSaved(await addTraveller(f));
    } catch (e) {
